...
export default function MetronomeBeatName({ id, fSize, noteDuration }) {
  let nameId;
  let color;
  ...
  if (noteDuration === "4n") {
    ...
    color = id % 2 === 0 ? "warning.main" : "text.primary"; // Основной счет — цвет "warning.main"
    ...
    if (positionInCycle === 0) {
      ...
      color = "text.primary"; // Обычный цвет для "та" и "и"
    } else {
      nameId = "та"; // Последняя "та"
      color = "text.primary"; // Обычный цвет
    }
  } else {
    // Дефолтное поведение
    nameId = id + 1;
