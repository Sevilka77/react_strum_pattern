...
export default function ControlBar({ config, onConfigChanged }) {
  ...
  return (
    <>
      <ToggleButtonGroup
        styled={{ border: 0, fontSize: "inherit" }}
        exclusive
        value={config.note}
      ...
      <ToggleButtonGroup onChange={handleContolChange} aria-label="text">
        <ToggleButton value="44" selected={config.isMetronomeSound}>
          {config.isMetronomeSound ? <VolumeXIcon /> : <Volume2Icon />}
        </ToggleButton>
        <ToggleButton value="45" selected={config.isBeatSound}>
          <DrumIcon />
        </ToggleButton>
        <ToggleButton value="46" selected={config.isPlaying}>
